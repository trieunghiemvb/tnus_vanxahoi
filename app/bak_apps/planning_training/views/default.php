...
<div class="col-sm-2">
    ...
    <!-- Kế thừa kế hoạch đào tạo cũ -->
    <div class="panel panel-warning">
        <div class="panel-heading" role="tab" id="headingTwo">
            <h4 class="panel-title">
                <a class="collapseOne" data-toggle="collapse" data-parent="#accordion" href="#collapseTwo" aria-expanded="true" aria-controls="collapseTwo">
                    Kế thừa kế hoạch đào tạo
                </a>
            </h4>
        </div>
        <div id="collapseTwo" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingTwo">
            <form class="form_filter">
                <div class="form-group" >
                    <label for="" class="control-label">Chọn khóa:</label>
                    <select name="sel_course_in" id="sel_course_in" class="form-control input-sm sel_filter">
                        <option value="0">-Tất cả-</option>
                        <?php
                        foreach ($this->arr_course as $key => $value) {
                            ?>
                ...
                <div class="form-group" >
                    <label for="" class="control-label">Chọn ngành học:</label>
                    <select name="sel_group_field" id="sel_group_field_in" class="form-control input-sm sel_filter">
                        <?php
                        foreach ($this->arr_group_field as $key => $value) {
                            ?>
                            <option value="<?= $key ?>"><?= $value ?></option>
                ...
                <div class="form-group" >
                    <label for="" class="control-label">Chọn chuyên ngành:</label>
                    <select name="sel_major" id="sel_major_in" class="form-control input-sm sel_filter">
                        <option value="0">-Tất cả-</option>
                        <?php
                        foreach ($this->arr_major as $key => $value) {
                            ?>
            ...
            <table id="action_table" class="table action_table">
                <tbody>
                    <tr align="center">
                        <td>
                            <button id="btn_inherit" class="btn btn-md btn-info btnSave"> Kế thừa kế hoạch <i class="glyphicon glyphicon-forward"></i></button>
                        </td>
                    </tr>
                </tbody>
            </table>
...
<script type="text/javascript">
    ...
    function load_major_in(id_group_field) {
        var ajaxUrl = "<?= AppObject::getBaseFile('app/planning_training/helpers/ajax.php') ?>";
        var str_post_data = id_group_field + ";;";
        $.post(ajaxUrl, {act: 'load_major', str_post_data: str_post_data}, function (str_response_data) {
            console.log("str_response_data=" + str_response_data);
            ...
            var str_major = data.str_major;
            var row_major = str_major.split('#');
            $('#sel_major_in').find('option:not(:first)').remove();
            var i = 0;
            $.each(row_major, function (key, value) {
                i++;
                var tmp = value.split(';');
                //console.log('tmp: '+ tmp);
                if (tmp[0] !== null && tmp[0] !== "") {
                    $('#sel_major_in').append('<option value="' + tmp[0] + '">' + tmp[1] + '</option>');
                }
            });
    ...
    function inherit_plan(id_course, id_course_in, id_major, id_major_in) {
        var ajaxUrl = "<?= AppObject::getBaseFile('app/planning_training/helpers/ajax.php') ?>";
        var str_post_data = id_course + ";" + id_course_in + ";" + id_major + ";" + id_major_in;
        $.post(ajaxUrl, {act: 'inherit_plan', str_post_data: str_post_data},
        function (str_response_data) {
            //console.log("str_response_data=" + str_response_data);
            var data = $.parseJSON(str_response_data);
            noticeMeseage.show(data.message, data.status);
    ...
    (function () {
        ...
        // Event select group_field_in
        $('.form_filter').on('change', '#sel_group_field_in', function () {
            console.log("Running!");
            modalLoading.show();
            var loaded = false;
            var id_group_field = $('.form_filter #sel_group_field_in').val();
            loaded = load_major_in(id_group_field);
            if (loaded)
                modalLoading.hide();
        });
        ...
        // Event click btn_inherit
        $('.action_table').on('click', '#btn_inherit', function () {
            var id_course = $('#form_filter #sel_course').val();
            var id_course_in = $('.form_filter #sel_course_in').val();
            var id_major = $('#form_filter #sel_major').val();
            var id_major_in = $('.form_filter #sel_major_in').val();
            inherit_plan(id_course, id_course_in, id_major, id_major_in);
        });
