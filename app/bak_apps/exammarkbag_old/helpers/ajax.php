...
switch ($act) {
    ...
    case 'loadfilter':
		...
		{							
			$room_list = ExamMarkBagApp::getExamRoomList($course, $exam, $subject);			
		}	
		else if($type_filter == 'room')
		{							
			$bag_list = ExamMarkBagApp::getExamBagList($course, $exam, $subject,$room);			
		}	
    ...
    case 'loadtester':
        ...
        $room = isset($_POST['room']) ? $_POST['room'] : "";
        $bag = isset($_POST['bag']) ? $_POST['bag'] : "";
        $testerList = ExamMarkBagApp::getExamTesterListByBag($id_exam, $room, $bag);
		$i = 0;
		$data = "";
		foreach ($testerList as $item )
		{
			...
			$data .= "<td class=\"beat\" style=\"width=80px; text-align:center;\" rowspan=\"1\" colspan=\"1\">".$item['beat']."</td>";
