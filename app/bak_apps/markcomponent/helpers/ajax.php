...
switch ($act) {
    ...
    case 'loadfilter':
		...
		{
			$term_list = MarkComponentApp::listTerm($course, $year);				
			$year_list = "";
			$subjects_list = "";
			$group_field_list = "";
			$classes_list = "";
		...
		{							
			$group_field_list = MarkComponentApp::listGroup_field();			
			$year_list = "";
			$term_list = "";
			$profile_list = "";
			...
			$classes_list = MarkComponentApp::listClass($course, $year, $term, $id_group_field, $id_subject);
			$year_list = "";
			$term_list = "";
			$group_field_list = "";
			$subjects_list = "";
		...
		if(is_array($year_list))
		{
			...
			{ 
				$year_data .='<option value="'.$value.'">'.$value.'</option>';
			} 
		...
		{			
			...
			foreach ($term_list as $key => $value){ 
				$term_data .='<option value="'.$value.'">'.$value.'</option>';
			} 
		...
		if(is_array($profile_list)){
			...
			/* if($term == "0")
			{ 
				$returndata .= '<option value="0">-- Tất cả --</option>';
			}else
			{
			} */
    ...
    case 'updatemark':
        ...
        $id_profile = isset($_POST['id_profile']) ? intval($_POST['id_profile']) : "";
        $mark_cc = isset($_POST['mark_cc']) ? $_POST['mark_cc'] : "";
        $mark_kt = isset($_POST['mark_kt']) ? $_POST['mark_kt'] : "";
        $mark_form = isset($_POST['mark_form']) ? $_POST['mark_form'] : "";
...
if($mark_form == 2){
	$mark_component = ($mark_cc +  $mark_kt)/ $mark_form;
	//$data = array('mark_cc' => $mark_cc, 'mark_kt' => $mark_kt, 'mark_component' => $mark_component);
	$sql = "UPDATE $table1 SET	$table1.`mark_cc` = $mark_cc, $table1.`mark_kt` = $mark_kt,	 $table1.`mark_component` = $mark_component WHERE $table1.`id_class_subject` = $id_class_subject AND $table1.`id_student` = $id_profile;";
	$result_mark = $db->query($sql, array(":mark_cc" => $mark_cc, ":mark_kt" => $mark_kt, ":mark_component" => $mark_component,":id_class_subject" => $id_class_subject, ":id_profile" => $id_profile));
}else{
	if($mark_cc == "" && $mark_kt !==""){
		$mark_component = $mark_kt;
		//$data = array('mark_kt' => $mark_kt, 'mark_component' => $mark_component);
		$sql2 = "UPDATE $table1 SET	$table1.`mark_kt` = $mark_kt,	 $table1.`mark_component` = $mark_component WHERE $table1.`id_class_subject` = $id_class_subject AND $table1.`id_student` = $id_profile;";
		$result_mark = $db->query($sql2, array(":mark_kt" => $mark_kt, ":mark_component" => $mark_component,":mark_component" => $mark_component, ":id_profile" => $id_profile));
	}else{
		$mark_component = $mark_cc;
		//$data = array('mark_cc' => $mark_cc, 'mark_component' => $mark_component);
		$sql3 = "UPDATE $table1 SET	$table1.`mark_cc` = $mark_cc,	 $table1.`mark_component` = $mark_component WHERE $table1.`id_class_subject` = $id_class_subject AND $table1.`id_student` = $id_profile;";
		$result_mark = $db->query($sql3, array(":mark_cc" => $mark_cc, ":mark_component" => $mark_component,":id_class_subject" => $id_class_subject, ":id_profile" => $id_profile));	}			
} 
		if($result_mark){
			$state = true;
