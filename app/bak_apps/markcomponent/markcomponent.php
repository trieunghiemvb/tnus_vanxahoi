...
class MarkComponentApp extends AppObject {
    ...
    public static function listSubject($year ="", $term="", $group_field="") {
		...
		if(!empty($term) && is_numeric($term))
            $condition['term'] = array(intval($term));
		if(!empty($group_field) && is_numeric($group_field) && ($group_field !== 0))
     ...
     * @return <array> trả về danh sách
     */
    public static function listClass($course, $year, $term, $group_field="", $subject="") {
        $db = new Database;
        $subjects = array();
        $class_subject = 'class_subject';
        $condition = array('status' => array('1'), 'id_course' => array($course), 'year' => array($year), 'term' => array($term));
