...
class MarkComponentApp extends AppObject {
    ...
    public static function listMarkComponent($id_class = "") {
        ...
        foreach ($result as $item) {
			...
			/* 
			$markComponent['student_code'] = self::getStudentCodeById($item['id_student']);
			$markComponent['id_profile'] = self::getProfileIdByIdStudent($item['id_student']);			
			$markComponent['class'] = self::getClassNameByIdStudent($item['id_student']);
			$result_profile = self::getProfileById($item['id_student']);
			foreach ($result_profile as $item_profile) {
				$markComponent['name'] = $item_profile['name'];
				$markComponent['sex'] = $item_profile['sex'];
				$markComponent['birthday'] = $item_profile['birthday'];
				$markComponent['birth_place'] = $item_profile['birth_place'];
