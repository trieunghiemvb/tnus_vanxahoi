...
// Bắt sự kiện thay đổi điều kiện lọc trên #form_filter
        $('#form_filter').on('change','.select_filter', function()
		{
			 ...
			 */if(this.id == 'filter_bag'){
				...
				$.post(ajaxUrl, {act: 'loadfilter', type_filter: "get_id_exam", course:course, exam:exam, subject:subject}, function(data){					
			...
			$.post(ajaxUrl, {act: 'loadfilter', type_filter: type_filter, course:course, exam:exam, subject:subject, bag:bag }, function(data) {
				//alert(data);
				var data = $.parseJSON(data);
                var str_data = data.returndata;				
...
// Event vào điểm thi của thí sinh
		...
		{            
			...
			$('#datatables tbody tr').each(function(){	
				...
				var postData = "Student:"+ student_id+" Mark:"+ mark_exam+" Course:"+ id_course+ " Exam Id: "+ id_exam_list_hidden + " Exam Name: "+ name_exam + " Subject: "+ id_subject + " Bag: "+ bag;
				//alert(postData);
				...
				$.post(ajaxUrl, {act: 'updatmark', id_student: student_id, mark_exam:mark_exam, id_course: id_course, name_exam:name_exam, id_exam: id_exam_list_hidden, id_subject:id_subject, bag: bag}, function(data) {				
					var data = $.parseJSON(data);	
					totalsv = totalsv + 1;					
...
// Event chọn phòng thi		
		$('.btnSelect').click(function()
		{
           ...
           // var room = $('#filter_room option:selected').val(); 
            var bag = $('#filter_bag option:selected').val(); 
            $('#course_selected').html($('#filter_course option:selected').text()); 
            $('#exam_selected').html(exam); 
            $('#subject_selected').html($('#filter_subject option:selected').text()); 
			...
			//$.post(ajaxUrl, {act: 'loadtester', id_exam:id_exam_list_hidden, bag:id_exam_bag_hidden}, function(data) 
			$.post(ajaxUrl, {act: 'loadtester', course:course, exam:exam, subject:subject, bag:bag}, function(data)
			{				
				//alert(data);					
				var data = $.parseJSON(data);
				var str_data = data.returndata;	
