	...
	case 'addnew':
		...
		if (empty($name) || empty($code)) {
			...
			if ($state) {
				...
				$status = 'danger';
				$message = 'Mã ngành nhập vào đã tồn tại trong cơ sở dữ liệu. Vui lòng thử lại';
			}
		}
	...
	case 'update':
		...
		if($id == 0){
			...
			if ($state) {
				...
				$status = 'danger';
				$message = 'Mã ngành nhập vào đã tồn tại trong cơ sở dữ liệu. Cập nhật không thành công. Vui lòng thử lại!';
			}
		}
		echo json_encode(array('status' => $status, 'message' => $message, 'returndata' => $returndata));
		break;
