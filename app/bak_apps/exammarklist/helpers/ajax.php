...
switch ($act) {
    ...
    case 'loadfilter':
		...
		{
			$exam_list = ExamMarkListApp::getExamNameListByIdCourse($course);						
		}else if($type_filter == 'exam')
		{
			$subjects_list = ExamMarkListApp::getExamSubjectList($course, $exam);				
		}else if($type_filter == 'subject')
		{							
			$room_list = ExamMarkListApp::getExamRoomList($course, $exam, $subject);			
		}	
		else if($type_filter == 'get_id_exam')
		{							
			$exam_id_selected = ExamMarkListApp::getExamIdList($course, $exam, $subject);			
		}	
    ...
    case 'loadtester':
        ...
        $room = isset($_POST['room']) ? $_POST['room'] : "";
        $testerList = ExamMarkListApp::getExamTesterList($id_exam, $room);
		$i = 0;
		$data = "";
		foreach ($testerList as $item )
		{
			...
			$data .= "<td class=\"number\" style=\"width: 16px;\" rowspan=\"1\" colspan=\"1\">".$i."</td>";
			$data .= "<td class=\"student_code\" style=\"width: 116px;\" rowspan=\"1\" colspan=\"1\">".$item['student_code']."</td>";
			$data .= "<td class=\"student_name\" style=\"width:130px;\" rowspan=\"1\" colspan=\"1\">".$item['student_name']."</td>";
			$data .= "<td class=\"student_birthday\" style=\"width:92px;\" rowspan=\"1\" colspan=\"1\">".$item['birthday']."</td>";
			$data .= "<td class=\"class\" style=\"width=110px;\" rowspan=\"1\" colspan=\"1\">".$item['class']."</td>";
			...
			//$data .= "<td class=\"state\" style=\"width:121px;\" rowspan=\"1\" colspan=\"1\"><input type=\"hidden\" value=\"".$item['status']."\" id=\"state_student_".$item['id_student']."\">".$option_state."</td>";
...
// cập nhật trạng thái thi của thí sinh
    case 'updatmark':
		...
		$term = ExamMarkListApp::getValueByKey($table_exam, 'term',$condition);		
		/* //get id_subject
		$condition = array('name' => array($id_subject), 'status' => array('1'));	
		$id_subject = ExamMarkListApp::getValueByKey('subject', 'id',$condition);		
 */
		...
		{			
			...
			if($result_mark){
				$get_mark_check = ExamMarkListApp::getValueByKey($table_class_subject_detail, 'mark_exam',$where);
