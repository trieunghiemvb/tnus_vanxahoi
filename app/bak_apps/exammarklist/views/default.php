...
?>
<link rel="stylesheet" href="<?php echo AppObject::getBaseFile('app/exammarklist/css/dataTables.bootstrap.css')?>">
<link rel="stylesheet" href="<?php echo AppObject::getBaseFile('app/exammarklist/css/class_style.css')?>">
<script src="<?php echo AppObject::getBaseFile('app/exammarklist/js/jquery.dataTables.min.js')?>"></script>
<script src="<?php echo AppObject::getBaseFile('app/exammarklist/js/dataTables.bootstrap.js')?>"></script>
<script src="<?php echo AppObject::getBaseFile('app/exammarklist/js/fnAddTr.js')?>"></script>
<script src="<?php echo AppObject::getBaseFile('app/exammarklist/js/scripts.js')?>"></script>
<script src="<?php echo AppObject::getBaseFile('app/exammarklist/js/jquery.download.js')?>"></script>
<style>
.message2{
...
<script type="text/javascript">
    ...
    (function(){
        ...
        var ajaxUrl = "<?=AppObject::getBaseFile('app/exammarklist/helpers/ajax.php')?>";
		// xóa thông báo
...
// Event cập nhật trạng thái thi của thí sinh
		...
		{            
			...
			$('#datatables tbody tr').each(function(){	
				...
				var mark_exam = $("#mark_input_"+student_id).val();

				//var mark_exam = $("#mark_exam_"+student_id).val();
							
				var postData = "Student:"+ student_id+" Mark:"+ mark_exam+" Course:"+ id_course+ " Exam Id: "+ id_exam_list_hidden + " Exam Name: "+ name_exam + " Subject: "+ id_subject + " Room: "+ id_room;
				//alert(postData);
				...
				$.post(ajaxUrl, {act: 'updatmark', id_student: student_id, mark_exam:mark_exam, id_course: id_course, name_exam:name_exam, id_exam: id_exam_list_hidden, id_subject:id_subject, room: id_room,}, function(data) {				
					var data = $.parseJSON(data);	
					totalsv = totalsv + 1;					
					if(data.status == 'success'){						
						...
						$("#mark_input_"+student_id).css('background', "#ccc");
						$("#mark_input_"+student_id).prop('disabled', true);
	
						var data_result = "Vào điểm thành công "+success_list+" thí sinh.";
					...
					else if(data.status == 'danger')
					{
						...
						$("#mark_input_"+student_id).css('background', "#fff");
						$("#mark_input_"+student_id).prop('disabled', false);
