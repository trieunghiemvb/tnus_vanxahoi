			...
			<div class="well">
				...
				<table id="action-table" class="table" style="display:none;">
					<tr>
						...
						<td>
							<div class="input-group">
								<input type="text" name="test_percent" id="test_percent" class="form-control input-sm" placeholder="Điểm thi" readonly>
								<span class="input-group-addon">%</span>
							</div>
						</td>
						<td>
			...
			<table class="table table-striped" id="datatable">
				...
				<tbody>
					...
					<tr id="formular_<?=$item->id?>">
						...
						<td class="formula" style="font-weight: bold;color: #f00;">TP*<?=($item->element_percent/100)?> + DT*<?=($item->test_percent/100)?></td>
						<td>
							<input type="hidden" name="control_str" class="control_str" value="<?=$item->id?>;<?=$item->id_course?>;<?=$item->element_percent?>;<?=$item->test_percent?>">
...
<script type="text/javascript">
	(function(){
		...
		// Update
		$('#action-table')
		.on('keyup', '#element_percent', function() {
			var element_percent = $(this).val();
			if (element_percent > 50) {
				element_percent = 50;
				$(this).val(element_percent);
			}
			test_percent = 100 - element_percent;
			$('#test_percent').val(test_percent);
		})
		.on('click', '.updateBtn', function() {
			...
			$.post(ajaxUrl, {act: 'update', id_course: id_course, element_percent: element_percent, id_formula: id_formula}, function(data) {
				console.log(data);
				...
				if (data.status == 'success') {
					...
					$('#formular_' + tmp[0] + ' .formula').html('TP*' + tmp[2]/100 + ' + DT*' + tmp[3]/100);
